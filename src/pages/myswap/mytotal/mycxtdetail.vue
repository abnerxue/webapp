<template>
  <div class="s-bg">
    <van-row class="s-headeras">
      ...
      <van-col span="20" style="text-align:center">常孝通明细</van-col>
    </van-row>
    <hr />
    ...
    <van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
      <div v-for="(item,i) in card" :key="i">
        <!-- <div class="list" @click="detail(item)"> -->
          ...
          <span>已锁定{{item.lock}}个常孝通</span>
          <span>开始解锁日期{{item.stime}}</span>
          
           <span>每天解锁比例{{item.rate}}</span>
...
export default {
  ...
  methods: {
    ...
    onLoad() {
      ...
      this.$ajax
        .post("/cxt/account/stock/lockDetail", that.$qs.stringify(data), {
